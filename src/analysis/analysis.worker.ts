...
//
// フリーズ対策（SPEC「重い解析でメインスレッドを塞がない」）の要。第 1 相（α マスク
// 構築、O(W×H)）に加えて、第 2 相もここで実行する。SPEC は「パラメータのみの変更は
...
//
// 第 1 相の成果物（α マスク、3000px 級で約 9MB）は Worker 内に imageId キーで保持し、
// メインスレッドへは送らない。第 2 相要求はパラメータだけを受け取り、キャッシュ済みの
// マスクから解析結果（間引き済みの輪郭数千点＝クローン軽量）だけを返す。カットラインの
// メモ（依存パラメータが同じ間の再利用）もキャッシュに同居させ、画像が変われば丸ごと捨てる。
//
...
/** 第 1 相要求：α マスクを構築してキャッシュし、完了を通知する。 */
