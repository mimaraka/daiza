 ...
 *
 * 原因はユーザーに見せても意味が薄く、また機微を含み得るため、UI へは定型文のみを
 * 出す。開発時の追跡のため、元の例外は console.error へ残す（外部送信はしない）。
 */
export function toUnexpectedError(cause: unknown): AnalysisError {
  ...
  return { kind: 'unexpectedError', message: UNEXPECTED_ERROR_MESSAGE };
}
