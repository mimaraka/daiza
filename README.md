...
- **台座幅の検査** — 支持多角形の考え方で、指定した台座幅で自立するかを安全率込みで判定
...
- **台座計算不可** — 指定した台座幅が必要幅（`max(2 × |重心X − 差込口中心X| × 安全率, 差込口幅)`）を下回る。台座幅は自動では広げず、パラメータの見直しを促します
...
- **第2相 `runAnalysis`** — パラメータに依存する計算（カットライン → 重心 → 差込部 → 台座 → 転倒角）。このうち重いカットライン生成（EDT膨張＋隙間埋め＋輪郭抽出）だけは依存パラメータを鍵にメモ化し、安全率スライダー等の無関係な変更では再計算しません。
