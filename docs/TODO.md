...
## 29. 連結部の接合部に隙間埋めが効かない（不具合修正）

隙間埋め（クロージング）はマスクに対して行われ、その**後**に `envelopeFromParts` がブリッジ矩形を
多角形 union で外形へ継ぎ足していた。そのためブリッジ側面とパーツ輪郭が作る入隅（接合点へ向かって
幅 0 まで細くなる隙間＝必ず閾値より狭い区間を含む）が充填対象から外れ、接合部が尖ったまま残って
いた（§24-1 で首部について直したのと同じ順序バグ）。`docs/SPEC.md`「隙間埋めと連結部の整合」を追加。

- [x] `analysis/distance.ts`：局所クロージング `closeMaskAround` を追加。円板クロージングは
      「足した材料から距離 2r 以内」しか結果が変わらず、各点の結果は「その点から距離 2r 以内」の
      マスクだけで決まる（外延性・冪等性から導かれる）。よってブリッジの外接矩形を 4r 広げた窓を
      切り出してクロージングし、2r 広げた範囲だけ書き戻せば全面クロージングと一致する。O(W×H) の
      重い段を二重に走らせないための最適化
- [x] `analysis/contour.ts`：`buildBridges` を「MST で連結（端点対）を選ぶ `bridgeLinks`」と
      「幅を与えて矩形化する `bridgeRects`」へ分解（最近点対の総当たりはリング頂点数の積に比例して
      重いため、マスク経路と多角形経路で結果を共有する）
- [x] `analysis/contour.ts`：`paintPolygon` を複数多角形対応（`paintPolygons`）にし、ブリッジを
      マスクへ塗り足してから周辺だけクロージングし直す `fillBridgeJunctions` を追加。連結の経路は
      `connectParts` に集約（隙間埋め有効＝マスク経路／無効・破綻時＝従来の多角形 union 経路）
- [x] ブリッジをマスクへ塗る際は左右へ 1px ずつ足す（`BRIDGE_RASTER_MARGIN_PX`）。輪郭追跡は画素
      中心を結ぶため塗った矩形は 1 辺あたり最大 1px 痩せる。SPEC は「最小幅を下回らない」ことだけを
      要求するので太る側へ倒す
- [x] 合成マスクで検証（Node）：離れた 2 円板（ギャップ 80px・閾値 40px・連結幅 30px）で、
      修正前は接合部に**深さ 4px・面積 176px の切り欠き**（＝クロージングで埋まるべき隙間）が残り、
      折れ角は 70°。修正後は残る凹みが**深さ 1px**（＝間引き許容値 `TRACE_SIMPLIFY_EPSILON_PX` 由来の
      量子化ノイズ。既存の隙間埋め円弧と同水準）まで下がり、折れ角は 26.6°（円弧を弦で刻んだ量）。
      連結部の実測幅は 32px（指定 30px 以上）。単一パーツ・隙間埋め無効（閾値 0）ではカットラインが
      修正前と完全一致（回帰なし）。2400×3000px・2 パーツで 444ms → 528ms（多パーツ時のみ）
